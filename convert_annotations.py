...
def convert_annotations(session_dir):
    ...
    if len(radar_files) != len(ann_data_sorted):
        print(
            f"⚠️ Frame count mismatch: {len(radar_files)} radar vs "
            f"{len(ann_data_sorted)} annotations in {session_dir}"
        )
...
def convert_all_to_ear(root_dir):
    """
    Recursively converts all Cascade_Capture_* radar folders in root_dir
    to EAR + RA format.
    """
    for radar_dir in find_radar_dirs(root_dir):
        convert_mat_to_ear_ra_npy(radar_dir)

    print(f"🎯 Done converting all radar sessions under {root_dir}")
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        root = config["paths"]["train_root"]
    else:
        root = config["paths"]["test_root"]
